...
| D-27 | Token-cost surface consolidation | Nine lifecycle tools (`spec_approve`/`spec_ratify`/`spec_claim`/`spec_close`/`spec_reopen`/`spec_park`/`spec_block`/`spec_unblock`/`spec_unpark`) collapsed into one `spec_transition` tool (`to:` enum), dropping the 20-tool roster to 12; `rootIndex` removed from every tool's workspace pick (rarely used, ~38% of schema footprint with `workspaceRoot`); `spec_task_check`'s legacy flat `phase`/`match`/`checked` form removed in favor of `items[]`-only; `spec_read` no longer returns a redundant parsed `frontmatter` (duplicate of `spec_md` bytes); `ok()` tool results are compact JSON instead of pretty-printed. Only consumers are the operator and LLM agents, so no external API stability guarantee applies and no back-compat shims were kept. | 031800ZJUL26 |
