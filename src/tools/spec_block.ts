...
export interface SpecBlockOutput {
  slug: string;
...
function injectBlockingSection(rawMd: string, reason: string): string {
  ...
  return [...before, blockingHeading, "", reason, "", ...after].join("\n");
}

export function specBlock(input: SpecBlockInput, ctx: ToolContext): SpecBlockOutput {
  if (!input.reason || input.reason.trim().length === 0) {
    throw new Error("reason_missing: spec_block requires a non-empty reason");
  }
